...
const salesmanData = [
    { name: 'Ahmed Khan', sales: 68400, orders: 52, commission: 6840 },
    { name: 'Sara Ali', sales: 59200, orders: 48, commission: 5920 },
    { name: 'Hassan Raza', sales: 52800, orders: 42, commission: 5280 },
    { name: 'Fatima Noor', sales: 45600, orders: 38, commission: 4560 },
    { name: 'Ali Akbar', sales: 38200, orders: 31, commission: 3820 },
];
            ...
            <div className="bg-gradient-to-br from-amber-50 to-orange-50 p-8 rounded-lg border-2 border-amber-200 shadow-md">
                <div className="flex items-center gap-4 mb-4">
                    ...
                    <div>
                        ...
                        <div className="text-2xl font-black text-redwood-text-main">{salesmanData[0].name}</div>
                    </div>
                </div>
                <div className="grid grid-cols-3 gap-4">
                    <div className="bg-white p-4 rounded-lg border border-amber-200">
                        <div className="text-[10px] font-bold text-redwood-text-muted uppercase tracking-wider">Sales</div>
                        <div className="text-2xl font-black text-redwood-text-main">${salesmanData[0].sales.toLocaleString()}</div>
                    </div>
                    <div className="bg-white p-4 rounded-lg border border-amber-200">
                        <div className="text-[10px] font-bold text-redwood-text-muted uppercase tracking-wider">Orders</div>
                        <div className="text-2xl font-black text-redwood-text-main">{salesmanData[0].orders}</div>
                    </div>
                    <div className="bg-white p-4 rounded-lg border border-amber-200">
                        <div className="text-[10px] font-bold text-redwood-text-muted uppercase tracking-wider">Commission</div>
                        <div className="text-2xl font-black text-emerald-600">${salesmanData[0].commission.toLocaleString()}</div>
                    </div>
                </div>
            </div>
                    ...
                    <tbody className="divide-y divide-redwood-bg-light">
                        {salesmanData.map((item, i) => (
                            <tr key={i} className="hover:bg-redwood-bg-light/30 transition-colors">
                                ...
                                <td className="px-6 py-4 text-right text-[14px] font-black text-emerald-600">${item.commission.toLocaleString()}</td>
                            </tr>
                        ))}
                    </tbody>
                </table>
